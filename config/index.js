  ...
  for (let i = 0; i < parts.length; i += 1) {
    if (result === undefined) {
      ...
      if (process.env.BUILD_FLAG_IS_DEV === 'true' && process.env.BUILD_FLAG_IS_CLIENT === 'true') {
        throw new Error(
          `${errorMessage} We have noticed that you are trying to access this` +
            ' configuration value from the client bundle (i.e. code that will ' +
            'be executed in a browser). For configuration values to be exposed ' +
